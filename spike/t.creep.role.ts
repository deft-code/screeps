...
import { CreepStats } from 't.creep';
...
export class CreepRole extends CreepStats {
  get intents(): any {
  ...
  get team(): Flag {
    return new Flag(this.memory.mission, COLOR_WHITE, COLOR_WHITE, this.mission!.roomName, 25, 25);
  }
  ...
  get atTeam() {
    return this.room.name === this.mission!.roomName
  }
  ...
  taskBoost(lab: StructureLab | null) {
    ...
    const err = lab.boostCreep(this.c);
...
// Forward declaration of a member supplied further down the chain (was: declare global Creep.moveNear).
