...
| `pioneer` | `Pioneer` | `rolePioneer` -> `roleBootstrap` | `role.bootstrap.js` | Same as startup, laid by the `Startup <room>` mission (not GlobalRespawn): spawned outside the mission room and homed on it, so `atHome`/`home` point at the assisted room. Also sent before the claim to build the planned roads/containers (`Startup.canPioneerEarly`). |
...
| `ctrl` | `Ctrl` | `roleCtrl` | `role.ctrl.js` | Sit on the `ctrl` spot, upgrade; refill from an adjacent store/link/container (remembers `memory.struct`). No `after`; the container on the spot is planned by `Meta_ctrl` and retired at RCL6 to decay ([metastruct.md](metastruct.md)). |
...
| `hub` | `Hub` | `roleHub` | `role.hub.ts` | Stationary on the `hub` spot: shuttle energy among storage, terminal, hub link, and adjacent spawn/tower using `storageBalance` and `hubNeed`; with an empty hand and nothing else to do, move non-energy stock from storage into the terminal while the terminal holds under 150k non-energy and under 5k of that resource (minerals in hand always go to the terminal first). `after`: renew in place (`idleImmortal`). |
