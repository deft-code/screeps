...
Creep
 ...
 └─ CreepRole    (creep.role.ts) @injecter  run/after dispatch, home/team/teamRoom/atTeam/atHome, checkMem/checkId/checkFlag, taskTask, boosts (taskNeedBoost/taskBoost*), nearSpawn, idleImmortal
...
| `worker` | `Worker` | `roleWorker` | `role.worker.js` | Build towers, turtle mode, build ordered, repair ordered, turtle prep/turtle, upgrade when storage > 10k or no storage; otherwise recharge (harvest fallback). |
