...
Creep.prototype.actionWithdraw = function(structure, resource, amount) {
    ...
    if(!structure) {
        ...
        if(resource) {
            structure = _(structures)
                .filter(s=>s.store[resource] > amount)
                .sample();
        } else {
            structure = _(structures)
                .filter(s=>s.storeTotal > amount)
                .sample();
        }
    }
    if(!resource) {
...
Creep.prototype.taskDoubleTime = function() {
    ...
    if(err != OK && err != ERR_NOT_ENOUGH_ENERGY && err != ERR_NOT_ENOUGH_RESOURCES) {
        delete this.memory.task.double;
        return false;
    }
    return false;
...
Creep.prototype.actionRecharge2 = function(lack) {
    ...
    return this.actionPickup(RESOURCE_ENERGY, lack) ||
        ...
        this.actionPickup(RESOURCE_ENERGY) ||
        this.actionUncharge() ||
