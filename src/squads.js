...
class WorkSquad extends Squad {
    execute() {
        ...
        if(this.spawn.spawning || room.energyAvailable < room.energyCapacityAvailable) {
            return "no spawning";
        }
...
StructureSpawn.prototype.newWorkSquad = function() {
    ...
    return this.newSquad(squad, WorkSquad, {rooms: [this.pos.roomName], nworkers: 1});
}
...
exports.run = () => {
    Game.squads = {};
    const squadNames = Object.keys(Memory.squads);
    for(let i in squadNames) {
        const name = squadNames[i];
        exports.initSquad(name);
    }
