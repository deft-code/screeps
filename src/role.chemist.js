...
modutil.cachedProp(StructureLab, "mineralFree", (lab) => lab.mineralCapacity - lab.mineralAmount);

function nonenergy(carry) {
    return _(_.keys(carry))
...
Creep.prototype.actionWithdrawMinerals = function(store) {
    ...
    return this.actionWithdraw(store, mineral);
}
...
Creep.prototype.taskWithdrawLab = function() {
    const lab = this.taskId;
    if(!lab || lab.mineralType !== this.memory.task.mineral || !lab.mineralAmount) {
        return false;
    }
    if(!this.carryFree) {
        return false;
