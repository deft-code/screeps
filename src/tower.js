var doTower = function(tower) {
    const struct = _(tower.room.cachedFind(FIND_STRUCTURES))
    ...
    const surpluss = tower.room.storage && tower.room.storage.store.energy > 500000;
    ...
    if((surpluss || dropped) && tower.energy > 800) {
        let need_repair = _.sample(tower.room.find(FIND_STRUCTURES, {
           filter: s => s.hitsMax - s.hits > 800 && s.structureType != STRUCTURE_ROAD && !s.memory.dismantle
        }), 3);
        if(need_repair.length) {
            const s = _(need_repair)
                .sortBy("repairs")
                .last();
            tower.repair(s);
            return s.note;
