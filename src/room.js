...
Object.defineProperty(Room.prototype, 'workCreeps', {
    get: function() {
        if(!this._workCreeps) {
            this._workCreeps = this.find(FIND_MY_CREEPS, {filter: c => c.memory.role == "worker"});
        }
...
let spawnUpkeep = function(spawn) {
    ...
    if(!counts.ranged && nhostiles) {
        let who = spawn.createRole([MOVE, RANGED_ATTACK, MOVE, RANGED_ATTACK, MOVE, RANGED_ATTACK, MOVE, RANGED_ATTACK], 2, 'ranged');
    ...
    W23S79(room) {
        ...
        if(!energies.length) {
            const extns = room.find(FIND_STRUCTURES, {
                filter: s => s.structureType == STRUCTURE_EXTENSION && s.owner.username != "deft-code"
            });
...
function upkeepMyRoom(room) {
    ...
    if(room.controller && room.controller.my) {
        const hostiles = room.cachedFind(FIND_HOSTILE_CREEPS);
        if(hostiles.length) {
            const hurt_towers = _.filter(
                room.Structures(STRUCTURE_TOWER),
                t => t.hits < t.hitsMax);
            const hurt_spawns = _.filter(
    ...
    if(false && room.name == "W27S79") {
        if(!Game.spawns.Second.spawning && room.energyCapacityAvailable == room.energyAvailable) {
...
function roomUpkeep(room) {
    ...
    let cleaned = _.remove(room.memory.objects, (id, mem) => _.isEmpty(mem) || Game.getObjectById(id) == null);
