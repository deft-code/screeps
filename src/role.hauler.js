...
Creep.prototype.actionPoolCharge = function() {
    ...
    const pools = _.filter(
        room.cachedFind(FIND_MY_SPAWNS).concat(room.Structures(STRUCTURE_EXTENSION)),
...
Creep.prototype.actionChargeAny = function() {
    ...
    let need = _(room.Structures(STRUCTURE_LAB).concat(room.Structures(STRUCTURE_TOWER)))
...
const creepNeedsCharge = (creep) =>
    creep.carryFree > creep.carry.energy &&
...
Creep.prototype.actionDischarge = function() {
    ...
    let workers = _.filter(
        ...
        w => w.carryFree > w.carry.energy && w.memory.task && w.memory.task.double == false);
...
Creep.prototype.actionDrain = function() {
    ...
    let links = _.filter(
        this.room.Structures(STRUCTURE_LINK),
        l => !l.memory.bucket);
...
Creep.prototype.taskUnstore = function() {
    ...
    let resource = this.memory.task.resource || 
        _.sample(Object.keys(src.store));
...
Creep.prototype.actionEmptyStore = function() {
    ...
    const store = _(conts)
        .filter(s => !s.store.energy && s.storeFree)
        .sample();
    if(!store) {
        return false;
    }
