...
Creep.prototype.actionHospital = function() {
    if(this.hits < this.hitsMax && !this.pos.inRangeTo(this.squad.home.controller, 5)) {
...
var rechargeAtLeast = function(creep, amount, withdraw) {
    ...
    let chargers = creep.room.find(FIND_STRUCTURES, {
        filter: s => s.charger && s.store[RESOURCE_ENERGY] > amount
    });
    
    let links = _.filter(creep.room.Structures(STRUCTURE_LINK), s => s.energy > amount);
    ...
    if(creep.room.storage && creep.room.storage.store[RESOURCE_ENERGY] > amount) {
        cs.push(creep.room.storage);
    }
    if(creep.room.terminal && creep.room.terminal.store[RESOURCE_ENERGY] > amount) {
...
var actionRecharge2 = function(creep, withdraw) {
    ...
    if(recharge.structureType == STRUCTURE_CONTAINER || recharge.structureType == STRUCTURE_STORAGE || recharge.structureType == STRUCTURE_LINK || recharge.structureType == STRUCTURE_TERMINAL){
...
let actionTransfer = function(creep) {
    if(!creep.memory.transfer) {
        if(!creep.room.needTransfer) {
            ...
            creep.room.needTransfer = creep.room.find(FIND_MY_STRUCTURES, {
...
var actionTransferTower = function(creep) {
    var targets = creep.room.find(FIND_STRUCTURES, {
        filter: s => s.structureType == STRUCTURE_TOWER && s.energy*2 < s.energyCapacity
    });
    if(targets.length) {
...
var actionRecharge = function(creep) {
    return actionRecharge2(creep, true) || actionWithdraw(creep) || actionPickup(creep) || actionHarvest(creep);
}
...
Creep.prototype.roleRanged = function() {
    ...
    const spawn = _.sample(this.room.Structures(STRUCTURE_SPAWN)) || Game.spawns.Third;
