...
| `Startup <room> [home]` mission | scheduled (`Startup W22S7`), same way | claiming a third room: scout, claimer, pioneers, guard until it has a tower. With GCL full it plans (and draws, yellow dashed) a Rewalker-costed road from the controller to the home spawn instead of laying a claimer, saves it as `rroad_<room>_startup` metas per room, and sends pavers to unowned rooms with sites; an invader core in the room draws a wolf every 1500 ticks |
